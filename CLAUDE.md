...
Use the inspector (`http://localhost:8400/inspector`, `pnpm dev` only) to call
...
1. **Modules are self-contained peers; the server is a thin composition root.**
   ...
   `create-module` skill walks through all of them.
...
code — never skip it. Widget changes additionally need a manual render check in
the inspector (`pnpm dev`, then call the `show_*` tool). The widget bundle is
read once at boot — after `pnpm build` restart `pnpm dev` to see UI changes,
or run `pnpm --filter ./server dev:ui` in a second terminal for rebuild-on-save.
